...
export default class Cart extends VuexModule {
  ...
  private ADD_ARTICLE(newArticle: Articles.Article): void {
    let previousArticle: Articles.Article = this.articles.filter(
  ...
  private ADD_MENU(newMenu: Articles.Menu): void {
    let previousMenu: Articles.Menu = this.menus.filter(
      (menu) => menu.id == newMenu.id
    )[0];
  ...
  private DELETE_ARTICLE(article: Articles.Article): void {
    this.articles = this.articles.filter(
      (item: Articles.Article) => item.id != article.id
    );
  }
  ...
  private INCREMENT_ARTICLE_QUANTITY(article: Articles.Article): void {
    this.articles.filter((item: Articles.Article) => item.id == article.id)[0]
      .quantity++;
  }

  @Mutation
  private INCREMENT_MENU_QUANTITY(menu: Articles.Menu): void {
    this.menus.filter((item: Articles.Menu) => item.id == menu.id)[0]
      .quantity++;
  }

  @Mutation
  private DECREMENT_ARTICLE_QUANTITY(article: Articles.Article): void {
    let existingArticle = this.articles.filter(
      (item: Articles.Article) => item.id == article.id
    )[0];
    existingArticle.quantity == 0
  ...
  private DECREMENT_MENU_QUANTITY(menu: Articles.Menu): void {
    let existingMenu = this.menus.filter(
      (item: Articles.Menu) => item.id == menu.id
    )[0];
    existingMenu.quantity == 0
  ...
  get totalPrice(): number {
    ...
    this.articles.forEach(
      (article: Articles.Article) => (total += article.price * article.quantity)
    );
    this.menus.forEach(
      (menu: Articles.Menu) => (total += menu.price * menu.quantity)
    );
    return total;
  }
  ...
  public async addArticle(
    newArticle: Articles.Article
  ): Promise<Array<Articles.Article>> {
  ...
  public async deleteMenu(
    newMenu: Articles.Menu
  ): Promise<Array<Articles.Menu>> {
    await this.DELETE_ARTICLE(JSON.parse(JSON.stringify(newMenu)));
    return this.menus;
  }

  @Action({ rawError: true })
  public async incrementArticleQuantity(
  ...
  @Action({ rawError: true })
  public async incrementMenuQuantity(
    menu: Articles.Menu
  ): Promise<Array<Articles.Menu>> {
  ...
  @Action({ rawError: true })
  public async decrementArticleQuantity(
    article: Articles.Article
  ): Promise<Array<Articles.Article>> {
    if (article.quantity < 1) {
      await this.DELETE_ARTICLE(JSON.parse(JSON.stringify(article)));
    } else {
      await this.DECREMENT_ARTICLE_QUANTITY(
  ...
  @Action({ rawError: true })
  public async decrementMenuQuantity(
    menu: Articles.Menu
  ): Promise<Array<Articles.Menu>> {
    if (menu.quantity < 1) {
